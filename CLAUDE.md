...
There is no test runner. The `tests/fixtures/` directory contains Markdown files for manual testing: `basic.md`, `code.md`, `math.md`, `table.md`, `mixed.md`.
...
- **`editor/plugins/`** — keymap, inputRules (Markdown shortcuts), history, search, dropImage, highlight
...
**CSS structure** — Three CSS files: `app.css` (layout/toolbar/tabs/welcome), `editor.css` (ProseMirror internals), `markdown.css` (content rendering — also inlined into HTML exports). Theme is toggled via `data-theme` attribute on `<html>`.
