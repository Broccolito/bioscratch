...
import { buildImageRenderPlugin, ImageSrcResolver } from "../editor/plugins/imageRender";
...
const EditorSurface: React.FC<EditorSurfaceProps> = ({
  ...
  useEffect(() => {
    ...
    const plugins = [
      // No prosemirror-history: VS Code owns undo/redo against the TextDocument.
      buildInputRules(),
      buildKeymap(
        () => onSaveRef.current(),
        () => onSearchRef.current(),
      ...
      buildImageRenderPlugin((src) => resolveSrcRef.current(src)),
      buildMermaidPlugin(),
    ...
    // numeric data, bypassing beforeinput entirely. Detect and delete it.
    // (We delete the injected characters directly rather than relying on
    // prosemirror-history, which is intentionally absent — VS Code owns undo.)
    const undoMathAutofill = (e: Event) => {
      ...
      if (
        ...
        const inserted = ie.data.length;
        // Wait one frame for ProseMirror to process the DOM mutation, then
        // remove the injected characters immediately before the cursor.
