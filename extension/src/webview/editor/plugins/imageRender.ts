...
// Host adapter: turns an image `src` (possibly relative to the document) into a
// URL the webview can load. In VS Code this resolves to a `vscode-resource` URI
// via `webview.asWebviewUri` on the extension host; remote/data URLs pass through.
...
function createBlockWidget(
  ...
  attachImage(renderedEl, alt, src, title, resolveSrc);
...
function createInlineWidget(
  ...
  attachImage(wrapper, alt, src, title, resolveSrc);
...
//
// In all cases the underlying paragraph text is unchanged, so undo, copy,
// and deletion all work exactly as for any other paragraph text.
export function buildImageRenderPlugin(resolveSrc: ImageSrcResolver): Plugin {
  return new Plugin({
    props: {
      decorations(state) {
        ...
        state.doc.descendants((node, pos) => {
          ...
          if (blockMatch) {
            ...
            decos.push(
              Decoration.widget(
                pos + node.nodeSize,
                (view) => createBlockWidget(alt, src, title, pos, isEditing, resolveSrc, view),
                {
                  side: 1,
          ...
          if (!isEditing) {
            for (const { pmStart, pmEnd, alt, src, title } of inlineMatches) {
              ...
              decos.push(
                Decoration.widget(
                  pmStart,
                  (view) => createInlineWidget(alt, src, title, pos, resolveSrc, view),
                  {
                    side: -1,
