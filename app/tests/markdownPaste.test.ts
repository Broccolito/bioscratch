...
describe("rich-text paste marks", () => {
  it("inherits and merges surrounding inline marks", () => {
    const cases = [
    ...
    for (const { surrounding, pasted, expected } of cases) {
      ...
      expect(result.text).toBe("123123213");
      expect(result.marksAtInsertedText).toEqual(expected.slice().sort());
    }
  });
});
