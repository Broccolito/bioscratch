...
export const enterForMarkdownBlocks: Command = (state, dispatch) => {
  ...
  // Away from the document's frontmatter position, Return commits thematic
  // break syntax just like Space does through the input rule.
  if (/^(?:---|\*\*\*|___)$/.test(dashNormalized)) {
    if (dispatch) {
...
export const enterInTableCell: Command = (state, dispatch) => {
  ...
  if (dispatch) {
    // Return is also a Markdown commit gesture. Convert links and inline math
    // before adding the cell's serializable <br>, just as Return does outside
    // a table before splitting the paragraph.
...
/**
 * Move between table cells. At the end of the final cell, create a new row and
 * move into it; at the beginning of the first cell, consume Shift-Tab so focus
 * never escapes the editor. Outside a table this deliberately returns false.
 */
export function moveTableCell(direction: 1 | -1): Command {
  return (state, dispatch) => {
    ...
    // There is no previous cell. Consuming the key is preferable to moving
    // browser focus to the toolbar or window chrome.
    ...
    if (dispatch) {
      ...
      if (rowTr) {
        ...
        if (table?.type.spec.tableRole === "table" && table.childCount > 0) {
          ...
          firstCellPos += 1; // enter the newly-added row before its first cell
          tr.setSelection(
